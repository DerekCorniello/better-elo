...
def train_test_split(dataset: List[UserGameData], test_size: float = 0.2, random_state: int = 0) -> Tuple[List[UserGameData], List[UserGameData]]:
...
def evaluate_baseline(test_dataset: List[UserGameData]) -> float:
    """Evaluate baseline MSE: predict no momentum adjustment (adjusted Elo = pre_game_elo)."""
    if not test_dataset:
        return 0.0
    mse = float(np.mean([(game.post_game_elo - game.pre_game_elo) ** 2 for game in test_dataset]))
    return mse


def run_evaluation(dataset: List[UserGameData], num_runs: int = 3, pop_size: int = 100, ngen: int = 50) -> Tuple[Any, List[float], List[float], List[float]]:
    ...
    for seed in range(num_runs):
        ...
        mses.append(mse)
        # Compute R2
        predictions = [game.pre_game_elo + game.momentum_adjustment for game in test]
...
def statistical_analysis(mses: List[float], baseline_mses: List[float], r2s: List[float]) -> Dict:
