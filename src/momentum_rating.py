...
class MomentumRating:
    ...
    def calculate_win_probability(self, opponent_elo: float) -> float:
        ...
        """
        return 1.0 / (1.0 + 10.0 ** ((opponent_elo - self.momentum_elo)
                                     / 400.0))

    def calculate_adaptive_K(self, features: List[float],
        ...
        if rating_trend_weight > 0 and features[3] > 0:  # positive trend
            momentum_multiplier += abs(features[3]
                                       * rating_trend_weight) / 100.0
...
class RatingCavityMetrics:
    ...
    def is_in_cavity(self, performance_gap: float,
    ...
    def calculate_performance_gap(self, actual_win_rate: float,
                                  expected_win_rate: float) -> float:
        """
        ...
        if expected_win_rate == 0 or expected_win_rate == 1:
            return 0.0
        elo_diff = -400 * \
            math.log10((1/expected_win_rate - 1) / (1/actual_win_rate - 1))
        return elo_diff
...
class TemporalValidator:
    ...
    def calculate_future_prediction_accuracy(test_dataset: List[Any],
        ...
        for game in test_dataset:
            ...
            momentum_rating.momentum_adjustment = sum(
                w * f for w, f in zip(momentum_weights,
    ...
    @staticmethod
    def detect_rating_cavities(dataset: List[Any],
                               momentum_weights: List[float]
